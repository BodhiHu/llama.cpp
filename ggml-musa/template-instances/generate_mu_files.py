    ...
    for type_k in TYPES_KV:
        for type_v in TYPES_KV:
            for head_size in get_head_sizes(type_k, type_v):
                with open(f"fattn-vec-f{vkq_size}-instance-hs{head_size}-{get_short_name(type_k)}-{get_short_name(type_v)}.mu", "w") as f:
...
for kq_acc_t in ["half", "float"]:
    for cols_per_block in [8, 16, 32]:
        ...
        with open(f"fattn-wmma-f16-instance-kq{kq_acc_t}-cpb{cols_per_block}.mu", "w") as f:
...
for type in TYPES_MMQ:
    with open(f"mmq-instance-{get_short_name(type)}.mu", "w") as f:
