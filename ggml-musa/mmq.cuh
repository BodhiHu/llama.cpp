...
static void launch_mul_mat_q(const mmq_args & args, musaStream_t stream) {
    ...
    if (!shmem_limit_raised[id]) {
        CUDA_CHECK(musaFuncSetAttribute(mul_mat_q<type, mmq_x, nwarps, false>, musaFuncAttributeMaxDynamicSharedMemorySize, shmem));
        CUDA_CHECK(musaFuncSetAttribute(mul_mat_q<type, mmq_x, nwarps, true>,  musaFuncAttributeMaxDynamicSharedMemorySize, shmem));
...
void mul_mat_q_case(const mmq_args & args, musaStream_t stream) {
    const int id    = ggml_cuda_get_device();
...
#define DECL_MMQ_CASE(type)                                                        \
    template void mul_mat_q_case<type>(const mmq_args & args, musaStream_t stream) \
